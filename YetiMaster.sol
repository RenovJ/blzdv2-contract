...
// xBLZDToken with Governance.
contract xBLZDToken is ERC20("BlizzardTokenV2", "xBLZD"), Ownable {
...
contract YetiMaster is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        // We do some fancy math here. Basically, any point in time, the amount of xBLZD
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (amount * pool.acccPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws want tokens to a pool. Here's what happens:
        //   1. The pool's `accxBLZDPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. xBLZD to distribute per block.
        uint256 lastRewardBlock; // Last block number that xBLZD distribution occurs.
        uint256 accxBLZDPerShare; // Accumulated xBLZD per share, times 1e12. See below.
        address strat; // Strategy address that will xBLZD compound want tokens
    ...
    uint256 public xBLZDPerBlock = 0.912 ether;
    uint256 public xBLZDDevPerBlock =  0.088 ether;
    ...
    // Update the given pool's xBLZD allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate,
    ...
    // View function to see pending xBLZD on frontend.
    function pendingxBLZD(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accxBLZDPerShare = pool.accxBLZDPerShare;
        ...
        if (block.number > pool.lastRewardBlock && wantLockedTotal != 0) {
            ...
            accxBLZDPerShare = accxBLZDPerShare.add(
                xBLZDReward.mul(1e12).div(wantLockedTotal)
            );
        }
        return user.amount.mul(accxBLZDPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 xBLZDReward =  multiplier.mul(xBLZDPerBlock).mul(pool.allocPoint).div(totalAllocPoint);

        xBLZDToken(xBLZD).mint(
            devaddr,
            multiplier.mul(xBLZDDevPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            )
        );
    ...
    function deposit(uint256 _pid,uint256 _wantAmt) public nonReentrant{
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accxBLZDPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
                safexBLZDTransfer(msg.sender, pending);
            }
        }
        if (_wantAmt > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accxBLZDPerShare).div(1e12);
    ...
    function safexBLZDTransfer(address _to, uint256 _xBLZDAmt) internal {
        uint256 xBLZDBal = IERC20(xBLZD).balanceOf(address(this));
        if (_xBLZDAmt > xBLZDBal) {
            IERC20(xBLZD).transfer(_to, xBLZDBal);
        } else {
            IERC20(xBLZD).transfer(_to, _xBLZDAmt);
        }
    }
